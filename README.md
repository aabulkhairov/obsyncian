# Obsync — affordable encrypted sync
...
- **Account required.** The plugin talks to the Obsync API (`obsync.nocodo.tech` by default) for authentication and sync metadata, and transfers file data directly to Cloudflare R2 object storage via short-lived signed URLs. No other network services are used.
- **Payment.** A free tier (1 GB) is available; larger storage requires a paid subscription purchased outside the plugin. Sync stops accepting new data when you exceed your quota; existing data stays downloadable.
...
- **Telemetry.** The plugin sends no telemetry. Server-side we keep operational logs (account email, request metadata, storage usage) as described in the [privacy policy](https://obsync.nocodo.tech/privacy).
...
1. Install and enable the plugin, then open **Settings → Obsync**.
...
Sync runs automatically on changes and on an interval; use the **Obsync: Sync now** command to force one.
